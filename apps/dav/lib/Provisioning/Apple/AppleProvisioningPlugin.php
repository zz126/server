...
class AppleProvisioningPlugin extends ServerPlugin {
	...
	public function httpGet(RequestInterface $request, ResponseInterface $response):bool {
		...
		$caldavUUID = UUIDUtil::getUUID();
		$carddavUUID = UUIDUtil::getUUID();
		$profileUUID = UUIDUtil::getUUID();
		...
		$body = vsprintf($xmlSkeleton, array_map(function($v) {
				...
				$description, $server_url, $userId, $useSSL, $serverPort, $principalUrl,
				$caldavDescription, $caldavDisplayname, $caldavIdentifier, $caldavUUID,
				$description, $server_url, $userId, $useSSL, $serverPort, $principalUrl,
				$carddavDescription, $carddavDisplayname, $carddavIdentifier, $carddavUUID,
